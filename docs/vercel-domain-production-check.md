...
**Domínio canônico oficial:** `https://reformaprofissional.com.br` (apex)  
...
- `vercel.json` direciona todo caminho de `www.reformaprofissional.com.br` para o mesmo caminho no apex, com redirect permanente. Não há rewrite nem segunda regra de host; portanto não há ciclo na configuração do repositório.
...
- O build injeta no HTML `<meta name="rp-release" content="<short-sha>">`. Na Vercel, a fonte é `VERCEL_GIT_COMMIT_SHA`; localmente, o fallback é `git rev-parse HEAD`. Assim, os dois hosts só podem ser considerados no mesmo release quando terminarem no apex e exibirem o mesmo marcador.
...
5. Abra esse deployment e confira **Domains/Aliases**. Os dois domínios devem pertencer a este mesmo projeto; o apex deve ser o alias de produção e `www` deve redirecionar para ele. Se um domínio estiver em outro projeto, remova-o do projeto antigo e associe-o a este projeto pelo painel, sem promover Preview.
...
- **`www`**: deve usar exatamente o CNAME recomendado pela Vercel, sem `A`, `AAAA`, URL forwarding ou CNAME concorrente. O redirect de `www` para apex é feito pelo deployment por `vercel.json`, não por dois encadeamentos de forwarding.
...
No apex, CSS deve ser `200` e `text/css`; JS deve ser `200` e MIME JavaScript (`text/javascript` ou `application/javascript`). SW, manifest e SVG devem ser `200` com MIME correspondente, nunca `text/html`. No `www`, é aceitável o redirect único para o mesmo path/query no apex; seguindo-o, o resultado deve cumprir os mesmos requisitos. Se HTML e asset exibirem releases/origens incompatíveis, purgue/reimplante somente após identificar qual alias atende o deployment incorreto.
...
4. Abra primeiro `https://reformaprofissional.com.br/`, mantenha **Disable cache** ativo no Network e recarregue.
5. Confirme que `/sw.js` responde `200` JavaScript, que o registro mostra scope `https://reformaprofissional.com.br/` e que CSS/JS não retornam HTML.
6. Abra `www`; ele deve redirecionar imediatamente para o apex sem instalar um novo worker em `www`.

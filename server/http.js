...
function wrap(fn){return async(req,res)=>{try{return await fn(req,res)}catch(error){console.error("[http] request failed",{method:req?.method,url:req?.url,statusCode:error.statusCode||500,code:error.message,stack:error.statusCode?undefined:error.stack});return send(res,error.statusCode||500,{error:error.statusCode?error.message:"INTERNAL_ERROR"})}}}
